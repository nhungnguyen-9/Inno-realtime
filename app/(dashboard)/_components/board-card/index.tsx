...
export const BoardCard = ({
  ...
  const toggleFavorite = () => {
    if (isFavorite) {
      ...
      onFavorite({ id, orgId })
        .catch(() => toast.error('Board already in favorite'))
    }
  }
    ...
    <Link href={`/board/${id}`}>
      <div className="group aspect-[100/127] border rounded-lg flex flex-col justify-between overflow-hidden">
        <div className="relative flex-1 bg-amber-50">
          ...
          >
            <button
              className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity px-2 py-1 outline-none"
            >
              <MoreHorizontal
